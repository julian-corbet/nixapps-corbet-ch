# llm-serving — the shared LLM SERVING lane (nixapps flagship). ONE llama-swap + ROCm llama.cpp broker
...
let
  ...
  modelVolumeMounts = map
    (d: { name = "models-${d}"; mountPath = "${cfg.storeMountPath}/${d}"; readOnly = true; })
    cfg.modelMounts;
    ...
    gpu = {
      ...
      hsaOverrideGfxVersion = lib.mkOption {
        ...
        description = ''
          ...
          supported architecture. The default, "10.3.0", is the value an RDNA2 consumer card needs —
          IT IS AN EXAMPLE, not a universal default. Find your own card's correct value from ROCm's
          supported-GPU list (or leave unset / empty string if your card is natively supported).
        '';
      };
    };
            ...
            spec = {
              ...
              initContainers = [
                ...
                # afterwards; the broker's --watch-config hot-reloads without a restart.
                {
                  ...
                  env = genEnv;
                  volumeMounts = [
                    { name = "config-d"; mountPath = "/config.d"; }
                  ] ++ modelVolumeMounts ++ [
              ...
              containers = [
                {
                  ...
                  command = [
                    "sh"
                    "-c"
                    ''D=$(dirname $(find / -name llama-server -type f 2>/dev/null | head -1)); export PATH=$D:$PATH; exec /shared/llama-swap --config /config.d/config.yaml --watch-config --listen 0.0.0.0:${toString cfg.brokerPort}''
                  ];
