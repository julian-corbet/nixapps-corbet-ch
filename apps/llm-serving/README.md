...
The shared LLM **serving lane** — the nixapps flagship. ONE llama-swap +
...
|---|---|---|---|
| `nixapps.llmServing.enable` | bool | `false` | Enable the module. |
| `nixapps.llmServing.namespace` | str | `"llm"` | Namespace for the broker and LiteLLM. |
| `nixapps.llmServing.createNamespace` | bool | `true` | Whether this application creates its namespace. |
| `nixapps.llmServing.project` | str | `"apps"` | nixidy AppProject. Map to whatever tier your scheme uses for apps that touch the GPU directly — this belongs with other direct GPU consumers, not with plain CPU-only apps or with nixgpu's own device-infra tier. |
| `nixapps.llmServing.modelStoreHostPath` | str | **required** | Host path to the model store root. No default — every deployment's storage differs; a default here would be a lie. |
| `nixapps.llmServing.modelMounts` | listOf str | `["embeddings" "rerankers"]` | Store subdirectories to bind-mount, one at a time (see above). `embeddings`/`rerankers` are structural — the generator's mode routing depends on those two names specifically. Add one entry per chat-model family you keep. |
| `nixapps.llmServing.storeMountPath` | str | `"/models"` | In-pod mount root for the model subdirectories (the generator's `STORE`). |
| `nixapps.llmServing.sysHostPath` | str | `"/sys"` | Host path bind-mounted so the generator can read live VRAM size. |
| `nixapps.llmServing.brokerPort` | port | `8080` | Port llama-swap listens on; also what LiteLLM's `api_base` targets. |

GPU contract surface (`nixapps.llmServing.gpu.*`):
...
Images (`nixapps.llmServing.images.*`), all pinned, source-observed defaults:
...
LiteLLM (`nixapps.llmServing.litellm.*`):
...
Generator (`nixapps.llmServing.generator.*`) — every threshold the store scan
...
```nix
{
  imports = [ inputs.nixapps.nixidyModules.llm-serving ];

  nixapps.llmServing.enable = true;
  nixapps.llmServing.modelStoreHostPath = "/srv/models"; # required — your real store root
  nixapps.llmServing.modelMounts = [ "embeddings" "rerankers" "chat" ];
  ...
  nixapps.llmServing.generator.aliases = {
