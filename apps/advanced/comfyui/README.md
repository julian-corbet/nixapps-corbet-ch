...
|---|---|---|---|
| `nixapps.advanced.comfyui.enable` | bool | `false` | Enable the module. |
| `nixapps.advanced.comfyui.namespace` | str | `"comfyui"` | Namespace for the Deployment and Service. |
| `nixapps.advanced.comfyui.appName` | str | `"comfyui"` | Name of the generated nixidy/Argo application; override to adopt an existing app name in-place during migration. |
| `nixapps.advanced.comfyui.createNamespace` | bool | `true` | Whether this application creates its namespace. |
| `nixapps.advanced.comfyui.project` | str | `"apps"` | nixidy AppProject. Map to whatever tier your scheme uses for apps that touch the GPU directly — not the plain CPU-only tier, and not nixgpu's own device-infra tier. |
| `nixapps.advanced.comfyui.modelStoreHostPath` | str | **required** | Host path to ComfyUI's whole `models/` tree (checkpoints/loras/controlnet/… as siblings) — ONE mount, unlike nixllm's per-subdirectory scheme, because ComfyUI itself expects the full conventional layout under one root. No default — every deployment's storage differs. |
| `nixapps.advanced.comfyui.modelMountPath` | str | `"/root/ComfyUI/models"` | In-pod mount path for the above — ComfyUI's own convention. |
| `nixapps.advanced.comfyui.stateHostPath` | str | **required** | Host path for the pod's persistent working directory (venv, `custom_nodes/`, cache). No default. |
| `nixapps.advanced.comfyui.stateMountPath` | str | `"/root"` | In-pod mount path for the above; also the base `preStartHookPath`/`modelMountPath` are relative to. |
| `nixapps.advanced.comfyui.outputHostPath` | str | **required** | Host path rendered images are written to. No default. Written as root — see the option's own doc on the resulting (harmless) ownership gap on a non-root-owned host directory. |
| `nixapps.advanced.comfyui.outputMountPath` | str | `"/output"` | In-pod mount path for the above; also becomes the `--output-directory` CLI value. |
| `nixapps.advanced.comfyui.httpPort` | port | `8188` | ComfyUI's listen port; also the Service's port. |
| `nixapps.advanced.comfyui.extraCliArgs` | listOf str | `[]` | Extra native CLI flags, appended after the always-present `--listen`/`--output-directory`. |
| `nixapps.advanced.comfyui.preStartHookPath` | str | `"/root/user-scripts/pre-start.sh"` | In-pod path the base image sources on every start, if present — `yanwk/comfyui-boot`'s own convention. |
| `nixapps.advanced.comfyui.preStartScript` | nullOr str | `null` | Optional startup hook content (see above); `null` renders no ConfigMap/init container at all. |
| `nixapps.advanced.comfyui.clusterIP` | nullOr str | `null` | Optional fixed ClusterIP. Leave `null` unless your routing needs a stable, pre-known VIP. |

Readiness (`nixapps.advanced.comfyui.readiness.*`):
...
Resources (`nixapps.advanced.comfyui.resources.*`):
...
GPU contract surface (`nixapps.advanced.comfyui.gpu.*`):
...
Wake-front consumer surface (`nixapps.advanced.comfyui.wake.*`):
...
Images (`nixapps.advanced.comfyui.images.*`):
...
```nix
{
  ...
  nixapps.advanced.comfyui.enable = true;
  nixapps.advanced.comfyui.modelStoreHostPath = "/srv/comfyui/models"; # required — your real store root
  nixapps.advanced.comfyui.stateHostPath = "/srv/comfyui/state";       # required
  nixapps.advanced.comfyui.outputHostPath = "/srv/comfyui/output";     # required

  # Optional: scale to zero behind a wake-front (bring your own nixgpu.ondemandFront entry).
  nixapps.advanced.comfyui.wake.enable = true;
  ...
  nixapps.advanced.comfyui.preStartScript = ''
    #!/bin/bash
    set -eu
...
```nix
{
  nixgpu.ondemandFront.apps.comfyui = {
    ...
    group = "comfyui"; # must match nixapps.advanced.comfyui.wake.sablierGroupLabelValue
    upstream = "comfyui.comfyui.svc.cluster.local"; # <namespace> matches nixapps.advanced.comfyui.namespace
    port = 8188; # matches nixapps.advanced.comfyui.httpPort
  };
}
```
